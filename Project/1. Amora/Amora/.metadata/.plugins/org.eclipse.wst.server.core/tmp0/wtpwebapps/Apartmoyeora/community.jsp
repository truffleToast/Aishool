...
<script>
	...
	document.addEventListener("DOMContentLoaded", function() {
		// 특정 시간(밀리초) 설정
		var delay = 500; // 1초 후에 스크롤 시작

		setTimeout(function() {
			// 스크롤할 위치의 Y 좌표 설정 (아래로 1500px 스크롤)
			var scrollY = 600;
				...
				<div class="row">
					<div class="topmargin">
						...
						<div class="board_wrap">
							...
							<div
								style="font-size: 24px; width: 1000px; border-top: 2px solid #000;"
								class="container">
								<div class="row">
									<!-- 글 출력되는 카테고리 란 -->
									<table class="table table-striped"
										...
										<tbody>
											...
											<tr style="background-color: white;">
												...
												<td><a id="sangsebogi"
													href="ViewCountUpService?com_Num=<%=list.get(i).getCom_Num()%>&com_likes=<%=list.get(i).getCom_Likes()%>"><%=list.get(i).getCom_Title()%>
												</a></td>
												<td><%=list.get(i).getNickname()%></td>
												<td><%=list.get(i).getCom_Date().substring(0, 11) + list.get(i).getCom_Date().substring(11, 13) + ":"
		+ list.get(i).getCom_Date().substring(14, 16)%></td>
									...
									%>
									<a style="border-color: #7297F7; background-color: #7297F7;"
										href="community.jsp?pageNumber=<%=pageNumber + 1%>"
